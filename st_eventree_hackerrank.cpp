...
int dfs(int v, bool visited[], vector<int> adj[]){
    visited[v]=true;
    int connectedcomponentcount=1; //to include current node
    vector<int>::iterator i;
    for(i=adj[v].begin();i!=adj[v].end();i++){
        if(!visited[*i]){
        int subtreecount=dfs(*i,visited,adj);

        if(subtreecount%2==0){
            mycount++;
        }

        connectedcomponentcount+=subtreecount;
        }
    }
...
int evenForest(int t_nodes, int t_edges, vector<int> t_from, vector<int> t_to) {
    ...
    //since it is  atree we don't have to call dfs on each vertex, calling on any one vertex would be sufficient
